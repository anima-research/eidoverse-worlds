...
import { gateStream, attachSource, detachSource, driveGate, gateOpenness,
         setMonitor, monitoring, release as releaseGate,
...
let _peerGen = 0;
// §24l R1 (survey A1): the onset machine and the level meter are ONE copy
// now — micgate.js's makeOnsetGate/makeLevelMeter, instantiated here with
// the mesh's own stream/mute authority. The ~200-line twin this replaced
// (and micstate.js's identical one) carried the production narrative in its
// comments; the machine took them along. The 🎙 announcement, the BasisVR
// noise floor, the hang-time and the settle window are all in the factory.
const _meter = makeLevelMeter(() => (!micStream || muted) ? null : (_rawMic || micStream));
export const micAnalyserLevel = () => _meter();
const _onset = makeOnsetGate({
  level: micAnalyserLevel,
  threshold: gateThreshold,
  // mute is authoritative: the transport's authority wraps the latch
  drive: (open) => driveGate((!micStream || muted) ? false : open),
...
export function toggleMute() {
  ...
  if (!muted) _onset.dropLatch();                   // gate decides from here
